...
def compute_features(profile_linear):
    """
    Compute shape-based surface classification features from near-field bins.
    These capture the visible differences in profile shape between surfaces.
    Input is linear-scale power values.
    """
    near = profile_linear[NEAR_BIN_START:NEAR_BIN_END].copy()

    if len(near) == 0 or np.max(near) == 0:
        return {'peak_power': 0, 'peak_width': 0, 'energy_ratio': 0,
                'rolloff': 0, 'roughness': 0}

    near_db = 10 * np.log10(near + 1)
    n = len(near_db)

    # 1. Peak Power (dB) — the actual maximum reflection strength
    peak_power = float(np.max(near_db))

    # 2. Peak Width — number of bins within 6 dB of peak (half-power width)
    #    Narrow = specular/smooth, Wide = scattering/rough
    half_power_thresh = peak_power - 6.0
    peak_width = float(np.sum(near_db >= half_power_thresh))

    # 3. Energy Ratio — fraction of total energy in first third vs last third
    #    Captures how front-loaded vs spread-out the reflection is
    third = max(n // 3, 1)
    energy_front = float(np.sum(near[:third]))
    energy_back  = float(np.sum(near[-third:]))
    energy_ratio = energy_front / (energy_back + 1e-6)

    # 4. Rolloff — average dB drop per bin in the 10 bins after the peak
    #    Steep rolloff = sharp specular reflection, gentle = diffuse scattering
    peak_idx = int(np.argmax(near_db))
    rolloff_end = min(peak_idx + 11, n)
    if rolloff_end > peak_idx + 1:
        rolloff_segment = near_db[peak_idx:rolloff_end]
        rolloff = float(rolloff_segment[0] - rolloff_segment[-1]) / len(rolloff_segment)
    else:
        rolloff = 0.0

    # 5. Roughness — std of residuals after removing a smooth trend
    #    High = textured/rough surface, Low = smooth/flat surface
    if n > 4:
        # Fit a 3rd-order polynomial as the smooth trend
        x = np.arange(n)
        trend = np.polyval(np.polyfit(x, near_db, 3), x)
        residuals = near_db - trend
...
def main():
    ...
    feat_keys    = ['peak_power', 'peak_width', 'energy_ratio', 'rolloff', 'roughness']
    feat_labels  = ['Peak Power (dB)', 'Peak Width (bins)', 'Energy Ratio', 'Rolloff (dB/bin)', 'Roughness']
    feat_colors  = ['#e74c3c', '#2ecc71', '#3498db', '#f39c12', '#9b59b6']
    ...
    while not exit_event.is_set():
        ...
        # Rescale left axis (peak_power)
        mp = list(feat_history['peak_power'])
        if mp:
            ax_feat.set_ylim(max(0, min(mp[-100:]) - 2), max(mp[-100:]) + 2)

        # Rescale right axis (other features)
        right_vals = []
        for k in ['peak_width', 'energy_ratio', 'rolloff', 'roughness']:

#!/usr/bin/env python3
"""
Grass Classifier — Live Inference with Autoencoder

Loads autoencoder_model.pth and classifies live radar range profiles.
Low reconstruction error = grass, high = not grass.

Usage:
    python autoencoder_classify.py
    python autoencoder_classify.py --model autoencoder_model.pth --threshold 0.05
"""
...
# ── Model (must match autoencoder_train.py) ──────────────────────
...
# ── ML Classification ───────────────────────────────────────────
def classify_grass_autoencoder(model, live_db, threshold):
    """Run autoencoder on ROI. Low error = grass, high error = not grass."""
    ...
    is_grass = error <= threshold

    return is_grass, error
...
# ── Main ─────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Live grass classifier (autoencoder)")
    parser.add_argument("--model", default="autoencoder_model.pth", help="Trained model path")
    parser.add_argument("--threshold", type=float, default=None,
                        help="Override reconstruction error threshold (default: from training)")
    ...
    trained_threshold = checkpoint["threshold"]
    ...
    model.eval()
    print(f"  Loaded ({n_bins} bins, threshold={threshold:.6f})")

    # ── Banner ───────────────────────────────────────────────────
    print("=" * 62)
    print("   xWRL1432 Grass Classifier — Autoencoder")
    print("=" * 62)
    print(f"   ROI:        {NEAR_START_M}–{NEAR_END_M} m  ({expected_bins} bins)")
    print(f"   Threshold:  {threshold:.6f}  (lower = stricter)")
    ...
    err_text = ax_result.text(0.5, 0.18, '',
                               transform=ax_result.transAxes, ha='center', va='center',
                               fontsize=13, color=TEXT_CLR)
    ax_result.set_title('Classification', color='white', fontsize=12,
                         fontweight='bold', pad=10)

    # Panel 3: Reconstruction overlay (ROI: original vs reconstructed)
    ax_recon = fig.add_subplot(gs[1, 0])
    style_ax(ax_recon, 'Reconstruction (ROI)')
    ax_recon.set_xlabel('Bin', color=TEXT_CLR, fontsize=9)
    ax_recon.set_ylabel('Normalized', color=TEXT_CLR, fontsize=9)

    # Panel 4: Error history
    ax_history = fig.add_subplot(gs[1, 1])
    style_ax(ax_history, 'Reconstruction Error History')
    ax_history.set_xlabel('Frame', color=TEXT_CLR, fontsize=9)
    ax_history.set_ylabel('MSE', color=TEXT_CLR, fontsize=9)

    fig.suptitle('xWRL1432 Grass Classifier — Autoencoder',
    ...
    while not exit_event.is_set():
        ...
        # ── Classify ─────────────────────────────────────────────
        is_grass, error = classify_grass_autoencoder(model, avg_profile, threshold)
        ...
        err_text.set_text(f'recon error: {error:.6f}  (threshold: {threshold:.6f})')
        err_text.set_color(TEXT_CLR)

        # ── Panel 3 & 4 (every 5 frames) ────────────────────────
        if frame_idx % 5 == 0:
            # Reconstruction overlay
            roi = avg_profile[NEAR_BIN_START:NEAR_BIN_END].astype(np.float32)
            roi_norm = (roi - roi.mean()) / (roi.std() + 1e-8)
            x_t = torch.tensor(roi_norm, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
            with torch.no_grad():
                recon = model(x_t).squeeze().numpy()

            ax_recon.clear()
            style_ax(ax_recon, 'Reconstruction (ROI)')
            ax_recon.set_xlabel('Bin', color=TEXT_CLR, fontsize=9)
            ax_recon.set_ylabel('Normalized', color=TEXT_CLR, fontsize=9)
            bins = np.arange(len(roi_norm))
            ax_recon.plot(bins, roi_norm, color='white', linewidth=1.5, alpha=0.8, label='input')
            ax_recon.plot(bins, recon, color=ACCENT, linewidth=1.5, alpha=0.8, label='reconstructed')
            ax_recon.fill_between(bins, roi_norm, recon, alpha=0.15, color='#e74c3c')
            ax_recon.legend(loc='upper right', fontsize=7, facecolor=BG_PANEL,
                            edgecolor=GRID_CLR, labelcolor='white')
            ...
            ax_history.set_xlabel('Frame', color=TEXT_CLR, fontsize=9)
            ax_history.set_ylabel('MSE', color=TEXT_CLR, fontsize=9)
            ax_history.axhline(y=threshold, color='#ff6b6b', linestyle='--',
                                alpha=0.5, label=f'threshold')

            hist = list(error_history)
            xs = list(range(len(hist)))
            colors_fill = ['#2ecc7140' if e <= threshold else '#e74c3c40' for e in hist]

#!/usr/bin/env python3
"""
Surface Classification via Range Profile — xWRL1432  (v3)
...
def classify_grass(live_db, ref_db):
    """Compare live ROI against grass reference. Returns (is_grass, similarity)."""
    live_roi = live_db[NEAR_BIN_START:NEAR_BIN_END]
    ref_roi  = ref_db[NEAR_BIN_START:NEAR_BIN_END]

    # Zero-mean before cosine similarity so we compare shape, not offset
    live_zm = live_roi - np.mean(live_roi)
    ref_zm  = ref_roi  - np.mean(ref_roi)

    sim = cosine_similarity(live_zm, ref_zm)
    return sim >= GRASS_THRESHOLD, sim
...
def main():
    ...
    while not exit_event.is_set():
        ...
        if grass_ref_db is not None:
            is_grass, sim = classify_grass(avg_profile, grass_ref_db)
            ...
            sim_text.set_text(f'similarity: {sim:.3f}  (threshold: {GRASS_THRESHOLD})')
